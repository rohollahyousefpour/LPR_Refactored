...
public:
    ...
    bool isNightTime() {
        ...
        time_t sunsetBuffer;
        get_buffere(sunriseBuffer, sunsetBuffer);
        const time_t twilightBeforeSunrise = 20 * 60;  // 20 minutes
        const time_t twilightAfterSunset = 30 * 60;

        // Returns true if current time is before sunrise or after sunset
        return (currentTime < sunriseBuffer - twilightBeforeSunrise
            || currentTime > sunsetBuffer + twilightAfterSunset);
    }
    ...
    void get_buffere(time_t& sunriseBuffer, time_t& sunsetBuffer);
    time_t  convertToTimeT(int year, int month, int day, double decimalHours);
    int get_delay(int ofset = 0);
