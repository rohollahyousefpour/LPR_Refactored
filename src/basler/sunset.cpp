...
bool SunSet::isTwilight() {
    time_t currentTime = std::time(nullptr);
    time_t sunriseBuffer;
    time_t sunsetBuffer;
    get_buffere(sunriseBuffer, sunsetBuffer);  // Assuming this function sets sunriseBuffer and sunsetBuffer to sunrise and sunset times respectively

    // Define twilight buffer times (in seconds): 20 minutes before sunrise and 30 minutes after sunset
    const time_t twilightBeforeSunrise = 20 * 60;  // 20 minutes
    const time_t twilightAfterSunset = 30 * 60;    // 30 minutes

    // Check if current time is within twilight period before sunrise or after sunset
    bool isBeforeSunriseTwilight = (currentTime >= (sunriseBuffer - twilightBeforeSunrise) && currentTime < sunriseBuffer);
    bool isAfterSunsetTwilight = (currentTime >= sunsetBuffer && currentTime <= (sunsetBuffer + twilightAfterSunset));

    return isBeforeSunriseTwilight || isAfterSunsetTwilight;
}
/**
...
void SunSet::get_buffere(time_t &sunriseBuffer, time_t &sunsetBuffer)
{
    const int bufferInSeconds = 0;// 5 * 60; // 30 minutes buffer
    time_t now = time(0); // Get current time
    tm* ltm = localtime(&now);

    // Set the current date and location for the SunSet object
    setCurrentDate(1900 + ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday);

    // Calculate sunrise and sunset times
    double sunrise = calcSunrise(); // Sunrise time in decimal hours
    double sunset = calcSunset(); // Sunset time in decimal hours
    time_t sunriseTime = convertToTimeT(ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday, sunrise);
    time_t sunsetTime = convertToTimeT(ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday, sunset);

    time_t currentTime = std::time(nullptr);
    sunriseBuffer = sunriseTime - bufferInSeconds;
    sunsetBuffer = sunsetTime - bufferInSeconds;
}
time_t  SunSet::convertToTimeT(int year, int month, int day, double decimalHours)
{
    // Fill in a tm struct with the date and decimalHours
    std::tm timeInfo = { 0 };
    timeInfo.tm_year = year; // Year since 1900
    timeInfo.tm_mon = month - 1;    // Month [0-11]
    timeInfo.tm_mday = day;         // Day of the month [1-31]

    // Calculate hours, minutes, and seconds from decimalHours
    int hours = static_cast<int>(decimalHours / 60);
    int minutes = static_cast<int>((decimalHours - hours * 60));
    int seconds = static_cast<int>((decimalHours * 60) - (hours * 3600) - (minutes * 60));
    ...
    // Make timeInfo represent local time
    timeInfo.tm_isdst = -1;
...
int SunSet::get_delay(int ofset)
{
    const int bufferInSeconds = 5 * 60; // 30 minutes buffer
    time_t now = time(0); // Get current time
    tm* ltm = localtime(&now);
    ...
    if (ofset == 0)
    {
        ...
        time_t sunsetTime_ = convertToTimeT(ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday, sunset);
        return (sunsetTime_ - now - bufferInSeconds) / 60;
    }
    else
    {
       
        setCurrentDate(1900 + ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday + 1);
        double sunrise = calcSunrise();
        time_t sunriseTime_ = convertToTimeT(ltm->tm_year, ltm->tm_mon + 1, ltm->tm_mday + 1, sunrise);
        return (sunriseTime_ - now - bufferInSeconds) / 60;
    }
}
