...
double jaroDistance(const std::string& a, const std::string& b)
{
    ...
    // If one std::string has null length, we return 0.
...
double jaroWinklerDistance(const std::string&a, const std::string& b)
{
    ...
    {
        // Calculate common std::string prefix.
        int commonPrefix(0);
        for (int index(0), indexEnd(Getmin<size_t>(Getmin<size_t>(a.size(), b.size()),(size_t) JARO_WINKLER_PREFIX_SIZE)); index < indexEnd; ++index)
        {
