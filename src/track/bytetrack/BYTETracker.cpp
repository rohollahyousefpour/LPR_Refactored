...
std::vector<STrack*> BYTETracker::update(const std::vector<Object>& objects, std::vector<STrack*>& lost_track)
{
	...
	while(this->removed_stracks.size()>5)
	...
	for (int i = 0; i < this->lost_stracks.size(); i++)
	{
		if (!this->lost_stracks[i].is_read_plate)
		{
			lost_track.push_back(&this->lost_stracks[i]);
		}
	}
