#include "lpr/track/bytetrack/STrack.h"

STrack::STrack(std::vector<float> tlwh_, float score, int label_)
{
	frame_set = false;
	label = label_;
	_tlwh.resize(4);
	...
	trace_rects.push_back(cv::Rect(_tlwh[0], _tlwh[1], _tlwh[2], _tlwh[3]));
...
void STrack::re_activate(STrack &new_track, int frame_id, bool new_id)
{
	...
	_tlwh[0] = new_track.tlwh[0];
	_tlwh[1] = new_track.tlwh[1];
	_tlwh[2] = new_track.tlwh[2];
	_tlwh[3] = new_track.tlwh[3];
	trace_rects.push_back(cv::Rect(_tlwh[0], _tlwh[1], _tlwh[2], _tlwh[3]));
...
void STrack::update(STrack &new_track, int frame_id)
{
	...
	if (this->tracklet_len > 0 && !get_label)
	{
		get_label = true;
		this->label_track = next_id2();
	}
